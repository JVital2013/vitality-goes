...
These files will probably not need to be configured by you, unless you're customizing what gets displayed. Some sample configs have disabled sections for composite imagery that is not generated by default (for example, Nighttime Microphysics in SatDump). If you enable these composites in the decoding program, you can easily enable them in your Vitality GOES config as well.
